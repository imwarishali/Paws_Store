...
if (isset($_SESSION['pending_user']['otp_time'])) {
    $time_elapsed = time() - $_SESSION['pending_user']['otp_time'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
    if (isset($_POST['resend'])) {
        if ($remaining_cooldown > 0) {
            ...
            $_SESSION['pending_user']['attempts'] = 0;
                            ...
                            <table width='600' cellpadding='0' cellspacing='0' style='background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);'>
                                ...
                                <tr>
                                    <td style='padding: 40px 30px;'>
                                        ...
                                        <p style='font-size: 16px; line-height: 1.5; color: #555555;'>We received a request to resend your registration code. Please use the following One-Time Password (OTP) to proceed:</p>
        ...
        if (empty($entered_otp)) {
        ...
        } elseif (time() - $_SESSION['pending_user']['otp_time'] > 600) { // 10 minutes expiry
            $error = "OTP has expired. Please register again.";
            unset($_SESSION['pending_user']);
        } elseif ($entered_otp != $_SESSION['pending_user']['otp']) {
            if (!isset($_SESSION['pending_user']['attempts'])) {
                $_SESSION['pending_user']['attempts'] = 0;
            }
            $_SESSION['pending_user']['attempts']++;

            if ($_SESSION['pending_user']['attempts'] >= 6) {
                $error = "Too many failed attempts. Please register again.";
                unset($_SESSION['pending_user']);
            } else {
                $remaining = 6 - $_SESSION['pending_user']['attempts'];
            ...
            try {
                ...
                $email = $_SESSION['pending_user']['email'];
                $phone = $_SESSION['pending_user']['phone'];
                $password_hash = $_SESSION['pending_user']['password'];

                // Re-check if the email or username was taken while waiting to verify
                $check_stmt = $pdo->prepare("SELECT id FROM users WHERE email = ? OR username = ?");
                $check_stmt->execute([$email, $username]);

                if ($check_stmt->rowCount() > 0) {
                    $error = "An account with this email or username was just created by someone else. Please try again.";
                    unset($_SESSION['pending_user']);
                } else {
                    ...
                    if ($insert_stmt->execute([$username, $email, $phone, $password_hash])) {
                        ...
                        // Send Welcome Email
                        $to = $email;
                        $subject = "Welcome to Paws Store! 🐾";
                        $message = "
                    <!DOCTYPE html>
                    <html>
                    <head>
                        <meta charset='UTF-8'>
                        <title>Welcome to Paws Store</title>
                    </head>
                    <body style='margin: 0; padding: 0; font-family: \"Helvetica Neue\", Helvetica, Arial, sans-serif; background-color: #faf7f2; color: #333333;'>
                        <table width='100%' cellpadding='0' cellspacing='0' style='background-color: #faf7f2; padding: 20px;'>
                            <tr>
                                <td align='center'>
                                    <table width='600' cellpadding='0' cellspacing='0' style='background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);'>
                                        ...
                                        <tr>
                                            <td style='padding: 40px 30px;'>
                                                <h2 style='color: #2c1a0e; margin-top: 0;'>Welcome, " . htmlspecialchars($username) . "!</h2>
                                                <p style='font-size: 16px; line-height: 1.5; color: #555555;'>We are absolutely thrilled to have you join the Paws Store family! 🎉</p>
                                                <p style='font-size: 16px; line-height: 1.5; color: #555555;'>Your account has been successfully created. You can now log in to explore our wide variety of lovely pets, save your favorites to your wishlist, and track your orders easily.</p>
                                                
                                                <div style='background-color: #fdfaf6; border: 1px solid #e8e0d4; border-radius: 8px; padding: 20px; margin: 30px 0; text-align: center;'>
                                                    <p style='margin: 0; font-size: 16px; color: #555555;'><strong>Ready to find your new best friend?</strong></p>
                                                </div>
                                                
                                                <p style='font-size: 16px; line-height: 1.5; color: #555555;'>If you have any questions or need assistance, our support team is always here for you.</p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style='background-color: #f9f9f9; padding: 20px; text-align: center; border-top: 1px solid #eeeeee;'>
                    ...
                    ";
                        ...
                        if (!empty($instance_id) && !empty($token) && strlen($clean_phone) >= 10) {
                            ...
                            $wa_body = "🐾 *Paws Store*\n\nWelcome, *" . htmlspecialchars($username) . "*! 🎉\n\nYour account has been successfully created. You can now log in to explore our wide variety of lovely pets, save your favorites to your wishlist, and track your orders easily.\n\nThank you for joining the Paws Store family!";
                        ...
                        unset($_SESSION['pending_user']);
                        $success = "Email verified and registration successful! Redirecting to login...";
...
<head>
    ...
    <title>Verify Email - Paws Store</title>
...
<body>
    <div class="register-wrap">
        ...
        <p class="sub">We have sent a 6-digit OTP to <strong><?php echo htmlspecialchars($_SESSION['pending_user']['email'] ?? ''); ?></strong> and your WhatsApp. Please enter it below.</p>
        ...
        <p class="login-link"><a href="register.php">Entered wrong email? Register again</a></p>

        <form method="POST" action="verify_otp.php">
            <input type="hidden" name="redirect" value="<?php echo htmlspecialchars($redirect); ?>">
            <input type="hidden" name="resend" value="1">
            <button type="submit" id="resend-btn" class="btn" style="background: #6c757d; margin-top: 10px;" disabled>Resend OTP</button>
        </form>
    </div>
